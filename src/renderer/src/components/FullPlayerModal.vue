...
const handlePlaylistPlayCurrent = () => {
  if (playerStore.currentIndex >= 0) {
    nextTick(() => {
      ...
      const currentElement = container.querySelector(`[data-index="${playerStore.currentIndex}"]`)
      if (currentElement) {
        ...
        <div class="full-player-content">
          ...
          <div class="player-main">
            ...
            <div class="album-cover">
              ...
              <div class="song-info">
                <h2 class="song-name">{{ playerStore.currentTrack?.name || '暂无歌曲' }}</h2>
                <p class="song-subtitle">
                  {{ playerStore.currentTrack?.album?.name || '' }}
                </p>
                <div class="song-tags">
                  <span v-if="playerStore.currentTrack?.quality === 'hq'" class="tag">HQ</span>
                  <span v-if="lyricsStore.currentLyrics.length" class="tag">LRC</span>
                  <span class="tag">{{ playerStore.currentTrack?.artists?.map((a: any) => a.name).join(' ') || '' }}</span>
                </div>
                <p class="song-album">
                  {{ playerStore.currentTrack?.album?.name || '' }} · {{ playerStore.currentTrack?.artists?.map((a: any) => a.name).join(' ') || '' }}
                </p>
              </div>
            </div>
            ...
            <div class="lyrics-area">
              <div ref="lyricsContainer" class="lyrics-container">
                <div v-if="lyricsStore.currentLyrics.length === 0" class="lyrics-empty">
                  {{ currentSongInfo }}
                </div>
            ...
            <div v-show="isMouseOver" class="bottom-controls">
              ...
              <div class="bottom-controls-row">
                ...
                <div class="bottom-center">
                  ...
                  >
                    <img v-if="playerStore.isPlaying" :src="PauseIcon" class="w-6 h-6" />
                    <img v-else :src="PlayIcon" class="w-6 h-6 transform translate-x-0.5" />
                  </button>
                ...
                <div class="bottom-right">
                  ...
                  <button
                    class="ctrl-btn"
                    :class="{ 'ctrl-btn-active': playerStore.isLyricsWindowVisible }"
                    title="歌词"
                    @click="playerStore.toggleLyricsWindow"
                  >
                    <img 
                      :src="playerStore.isLyricsWindowVisible ? LyricsCloseIcon : LyricsOnIcon"
                      class="w-5 h-5"
                    />
                  </button>
                  ...
                  >
                    ...
                    >
                      <div class="volume-panel-inner">
                        ...
                        <!-- 音量百分比（在下方） -->
                        <span class="volume-text">{{ playerStore.isMuted ? 0 : playerStore.volume }}%</span>
                      </div>
                    </div>
                  </div>
          ...
          <Transition name="fade">
            <div v-if="showPlaylist" class="playlist-modal">
              ...
              <div class="playlist-panel">
                ...
                <div ref="playlistScrollContainer" class="playlist-content">
                  ...
                  <div v-else>
                    <div
                      ...
                      class="playlist-item"
                      :class="{ 'playlist-item-active': index === playerStore.currentIndex }"
                    >
                      <!-- 序号/播放图标 -->
                      <div class="playlist-item-index">
                        <span v-if="index === playerStore.currentIndex" class="playlist-playing">▶</span>
                        <span v-else class="playlist-number">{{ index + 1 }}</span>
                      </div>
                ...
                <!-- 底部操作栏 -->
                <div class="flex items-center justify-between p-4 border-t border-theme">
                  <button
