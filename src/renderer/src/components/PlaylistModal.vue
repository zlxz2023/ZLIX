  ...
  <Teleport to="body">
    <Transition name="playlist">
      <div v-if="playerStore.showPlaylist" class="fixed inset-0 z-[100]">
        ...
        <!-- 右侧抽屉 -->
        <div class="absolute top-0 right-0 bottom-0 w-96 bg-secondary shadow-2xl flex flex-col select-none border-l border-theme">
        <!-- 头部 -->
        ...
        <div ref="scrollContainer" class="flex-1 overflow-y-auto overflow-x-hidden">
          <div v-if="playerStore.playlist.length === 0" class="py-12 text-center text-muted">
            暂无播放歌曲
          </div>
          ...
          <div v-else>
            <div
              ...
              :class="{
                ...
                'hover:bg-hover hover:scale-105': index !== playerStore.currentIndex
              }"
            >
              <!-- 序号/播放图标 -->
              <div class="w-8 flex items-center justify-center">
                <span v-if="index === playerStore.currentIndex" class="text-accent text-lg">▶</span>
              ...
              <!-- 时长 -->
              <span class="text-secondary text-xs">{{ formatDuration(track.duration) }}</span>
        ...
        <!-- 底部操作栏 -->
        <div class="flex items-center justify-between p-4 border-t border-theme bg-primary">
          <button
            @click="handleClear"
