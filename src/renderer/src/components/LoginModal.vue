...
import { getLoginQrKey, checkLoginQr, sendSmsCode, loginBySms, createWxLogin, checkWxLogin, loginByOpenPlat } from '@renderer/api/kugou/user';
...
const initWxLogin = async () => {
  try {
    ...
    const hasQrcodeBase64 = res && res.qrcode && typeof res.qrcode === 'object' && res.qrcode.qrcodebase64;
    ...
    <div v-if="isVisible" class="login-modal-overlay" @click.self="emitClose">
      ...
      <div v-else-if="loginMethod === 'wx'" class="qr-login">
        ...
        <div v-else-if="wxStatus === 'ready'" class="qr-content">
          ...
          <p>使用微信扫码登录</p>
          <button class="refresh-btn" @click="refreshWx">
            刷新二维码
          </button>
        </div>
