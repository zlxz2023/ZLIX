...
import { claimDayVip, upgradeDayVip, getUserVipDetail, getVipMonthRecord } from '@renderer/api/kugou/user';
...
const loadData = async () => {
  ...
  try {
    ...
    const busiVip = vipDetail.value?.busi_vip || [];
    const isSvipActive = !!busiVip.find((v: any) => v.product_type === 'svip' && v.is_vip === 1);
...
const handleClaimVip = async () => {
  if (isClaiming.value || userStore.isTvipClaimedToday)
    return;
...
const getVipExpireText = (vipData: any) => {
 ...
 if (days > 30)
 return `${Math.floor(days / 30)}个月后到期`;
 if (days > 0)
 return `${days}天后到期`;
 return '即将到期';
 }
      ...
      <div class="user-center-modal">
          ...
          <div class="avatar-section">
            <div class="avatar-wrapper">
              <img 
                ...
                class="avatar"
              />
              <span v-else class="avatar-placeholder">{{ getNickname.charAt(0) }}</span>
            </div>
        ...
        <div class="main-content">
          ...
          <div class="daily-benefits">
            ...
            <div class="benefit-item claim-vip" :class="{ claimed: userStore.isTvipClaimedToday }" @click="handleClaimVip">
              ...
              <div class="benefit-status">
                <span v-if="userStore.isTvipClaimedToday" class="claimed-badge">✓</span>
                <span v-else-if="isClaiming" class="claiming-badge">...</span>
                <span v-else class="claim-arrow">→</span>
              </div>
            </div>
            
            <div class="benefit-item upgrade-vip" :class="{ upgraded: userStore.isSvipClaimedToday || svip }" @click="handleUpgradeVip">
              ...
              <div class="benefit-status">
                <span v-if="userStore.isSvipClaimedToday || svip" class="claimed-badge">✓</span>
