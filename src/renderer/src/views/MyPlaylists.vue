...
<template>
  <div class="h-full flex flex-col bg-secondary select-none">
        ...
        <div class="flex items-center gap-3">
          ...
          <template v-else>
            ...
            <button 
              ...
              @click="selectAll"
            >
              {{ selectedIds.size === playlists.filter(p => !p.isDefault).length ? '取消全选' : '选择全部' }}
            </button>
            <button 
          ...
          <div class="relative w-full aspect-square rounded-xl mb-3 overflow-hidden">
            ...
            <div v-if="editMode && !item.isDefault" class="absolute top-2 left-2 z-10">
              <div 
                ...
                :class="selectedIds.has(item.id) ? 'bg-red-500 border-red-500' : 'bg-active/80 border-theme'"
              >
            ...
            <!-- 默认歌单标签 -->
            <div v-if="item.isDefault" class="absolute bottom-2 left-2 bg-black/50 text-white text-xs px-2 py-1 rounded">
              默认
            </div>
          </div>
    ...
    <!-- 新建歌单弹窗 -->
    <div v-if="myPlaylistStore.showCreateModal" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50" @click.self="myPlaylistStore.closeCreateModal()">
