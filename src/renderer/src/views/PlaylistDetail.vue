...
<template>
  <div class="playlist-detail">
    ...
    <div v-else-if="isLocalPlaylist && localPlaylist" class="content">
      ...
      <div class="tracks">
        <div v-if="!localPlaylist.tracks || localPlaylist.tracks.length === 0" class="empty-track">
          <span>暂无歌曲</span>
        </div>
        <table v-else class="track-table">
          ...
          <tbody class="cursor-pointer">
            <tr v-for="(track, index) in localPlaylist.tracks" :key="track.id" class="track-row transition-all duration-200 hover:scale-105">
              ...
              <td class="track-actions">
                ...
                <button class="btn-icon" @click="handleRemoveFromPlaylist(track.id)">
    ...
    <div v-else-if="playlist" class="content">
      ...
      <div class="tracks">
        <table class="track-table">
          ...
          <tbody class="cursor-pointer">
            <tr v-for="(track, index) in playlist.tracks" :key="track.id" class="track-row transition-all duration-200 hover:scale-105">
              <td class="track-index">{{ index + 1 }}</td>
              <td class="track-name">
                <div class="track-cover">
                ...
                <div class="track-info">
                  <span class="track-title">{{ track.name }}</span>
                  <span class="track-artists">{{ track.ar.map(a => a.name).join(' / ') }}</span>
                </div>
              </td>
              ...
              <td class="track-duration">{{ formatDuration(track.duration || (track as any).dt) }}</td>
            </tr>
          </tbody>
        </table>
      </div>
            ...
            >
              <div class="w-10 h-10 rounded-lg overflow-hidden flex-shrink-0">
                <div v-if="playlistItem.picUrl" class="w-full h-full">
                  <img :src="playlistItem.picUrl" class="w-full h-full object-cover" />
                </div>
              ...
              <div class="flex-1">
                <p class="text-gray-800 text-sm font-medium">{{ playlistItem.name }}</p>
                <p class="text-gray-400 text-xs">{{ playlistItem.trackCount }} 首歌曲</p>
              </div>
            </div>
          </div>
        </div>
