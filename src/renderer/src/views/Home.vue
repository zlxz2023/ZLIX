    ...
    <template v-else>
      ...
      <section class="mb-10">
        <div class="flex items-center justify-between mb-6">
          ...
          <button @click="goToDiscoverPlaylist" class="text-secondary hover:text-primary transition-all text-sm cursor-pointer">查看更多</button>
        </div>
        ...
        <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6 gap-5">
          ...
          >
            <div class="relative w-full aspect-square rounded-xl mb-3 overflow-hidden shadow-sm group-hover:shadow-md transition-shadow duration-200">
              <img
                v-if="item.picUrl"
                :src="item.picUrl"
                :alt="item.name"
      ...
      <section class="mb-10">
        <div class="flex items-center justify-between mb-6">
          ...
          <button @click="goToDiscoverArtist" class="text-secondary hover:text-primary transition-colors text-sm cursor-pointer">查看更多</button>
        </div>
        ...
        <div class="grid grid-cols-3 md:grid-cols-4 lg:grid-cols-6 xl:grid-cols-8 gap-5">
          ...
          >
            <div class="w-full aspect-square rounded-full mb-3 overflow-hidden shadow-sm group-hover:shadow-md transition-shadow duration-200">
              <img
                v-if="item.picUrl"
                :src="item.picUrl"
                :alt="item.name"
                class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300"
              />
              <div v-else class="w-full h-full bg-gradient-to-br from-purple-100 to-pink-100 flex items-center justify-center text-3xl">
                🎤
              </div>
            </div>
            <h3 class="text-primary text-sm truncate">{{ item.name }}</h3>
            <p v-if="item.albumSize" class="text-secondary text-xs">{{ item.albumSize }}张专辑</p>
          </div>
        </div>
      </section>
      ...
      <section>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
          ...
          >
            <div class="relative w-full aspect-video rounded-xl mb-3 overflow-hidden shadow-sm group-hover:shadow-md transition-shadow duration-200">
              <img
                v-if="item.cover"
                :src="item.cover"
                :alt="item.name"
              ...
              />
              <!-- 时长 -->
              <div v-if="item.duration" class="absolute bottom-2 right-2 bg-black/50 text-white text-xs px-2 py-1 rounded">
            ...
            <p v-if="item.artistName" class="text-secondary text-xs mt-1">{{ item.artistName }}</p>
          </div>
        </div>
      </section>
    </template>
