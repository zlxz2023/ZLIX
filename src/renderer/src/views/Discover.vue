...
const handleSongClick = (item: any) => {
    const track: Track = {
        ...
        artists:
            ...
            item.ar?.map((a: any) => ({ id: a.id, name: a.name })) ||
            [],
        album: {
            id: item.album?.id,
...
<template>
    <div class="h-full flex flex-col bg-secondary select-none">
        ...
        <div class="flex-1 overflow-y-auto px-8 pb-6">
                            ...
                            >
                                ...
                                <span>{{
                                    (item.playCount || 0) >= 10000
                                        ? ((item.playCount || 0) / 10000).toFixed(1) + '万'
                                        : item.playCount
                                }}</span>
                            </div>
                        </div>
            ...
            <div v-if="activeTab === 'artist'">
                ...
                <div class="grid grid-cols-4 md:grid-cols-6 lg:grid-cols-8 gap-2">
                    ...
                    >
                        ...
                        <span class="text-primary text-sm truncate max-w-full text-center">{{
                            item.name
                        }}</span>
                    </div>
                </div>
                        ...
                        >
                            ...
                            <td class="track-name">
                                <div class="track-cover">
                                    ...
                                    <div
                                        v-else
                                        class="w-full h-full bg-gray-100 flex items-center justify-center text-lg"
                                    >
                                        🎵
                                    </div>
                                </div>
                                <div class="track-info">
                                    ...
                                    <span
                                        class="track-artists"
                                        >{{ item.artists?.map((a: any) => a.name).join(' / ') || '' }}</span
                                    >
                                </div>
                            </td>
                            ...
                            <td class="track-duration">
                                {{
                                    formatDuration(
                                        item.duration || item.dt || item.mMusic?.playTime || 0
                                    )
                                }}
                            </td>
                        </tr>
            ...
            >
                <div
                    class="bg-secondary rounded-2xl w-full max-w-lg mx-4 overflow-hidden shadow-xl border border-theme"
                >
