  ...
  <aside class="w-66 bg-secondary flex flex-col h-full border-r border-theme-light select-none">
    ...
    <div class="px-4 pt-8">
      ...
      >
        ...
        >
          <div class="w-11 h-11 rounded-full bg-gradient-to-br from-purple-500 to-pink-500 flex items-center justify-center flex-shrink-0 overflow-hidden">
            <img 
              v-if="getUserPic()" 
              :src="getUserPic()" 
              alt="头像" 
            ...
            />
            <span v-else class="text-white text-xl font-bold">{{ getNickname().charAt(0) }}</span>
          </div>
          <div class="flex-1 min-w-0">
            <p class="font-medium text-primary truncate">{{ getNickname() }}</p>
            <p v-if="getUserLevel()" class="text-sm text-secondary">{{ getUserLevel() }}</p>
    ...
    <nav class="flex-1 py-4">
      ...
      <div
        ...
        :class="activeItem === item.path ? 'bg-active text-primary scale-105' : 'text-secondary hover:bg-hover hover:text-primary hover:scale-105'"
